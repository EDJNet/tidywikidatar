...
- connection:
  ...
  Defaults to `NULL`. If `NULL`, uses local SQLite database. If given,
  must be a connection object or a list with relevant connection
  settings (see example).

- RSQLite:

  Defaults to `NULL`, expected either `NULL` or logical. If set to
  `FALSE`, details on the database connection must be given either as a
  named list in the connection parameter, or with
