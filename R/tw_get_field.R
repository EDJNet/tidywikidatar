...
#' with [tw_get()]
#'
#' @param df A data frame typically generated with [tw_get()]. It should include
#'   data for the `id` included in the dedicated parameter.
#' @param field A character vector of length one. Typically, either "label" or
#'   "description".
#' @param id A character vector, typically of Wikidata identifiers. The output
#'   will be of the same length and in the same order as the identifiers
#'   provided with this parameter.
#' @inheritParams tw_get
#'
#' @return A character vector of the same length, and with data in the same
#'   order, as `id`.
#' @export
#'
#' @examples
...
tw_get_field <- function(
  ...
  field_df <- df %>%
    dplyr::filter(
      ...
      stringr::str_ends(
        string = .data$property,
        pattern = stringr::str_c(language, collapse = "|")
      )
    ) %>%
